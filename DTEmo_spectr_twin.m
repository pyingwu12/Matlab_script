...
clear;  ccc=':';
%---
expri='TWIN025';  expri1=[expri,'Pr001qv062221'];  expri2=[expri,'B']; 
...
for ti=1:lenhr
  ...
  %---calculate---
    for li=1:nzi
      ...
      p.perfft=fft2(p.diff(:,:,li));


    %---calculate KE (power of the FFT)---
    % u.diff.^2 + v.diff.^2 + cp/Tr*t.diff.^2 + R*Tr*(P.diff/Pr).^2 + Lv^2/cp/Tr*qv.diff.^2 );
       KE.twoD(:,:,li) = (abs(u.fft).^2 + abs(v.fft).^2 + cp/Tr*abs(t.fft).^2 + R*Tr*abs(p.fft).^2/Pr^2 + Lv^2/cp/Tr*abs(qv.fft).^2)/nx/ny ;  %2-D low level mean      
       KEp.twoD(:,:,li) = (abs(u.perfft).^2 + abs(v.perfft).^2 + cp/Tr*abs(t.perfft).^2 +  R*Tr*abs(p.perfft).^2/Pr^2 + Lv^2/cp/Tr*abs(qv.perfft).^2)/nx/ny ;          
    end %li=lev
    %--shift
