...
member=8; hr=0:17; minu=0:10:50;   %initial time: 10/12 0000 UTC
...
%       pmsl= movmean(pmsl0,9); % running mean
      pmsl=movmean(movmean(pmsl0,100),100,2);
      %--distance from fg
...
%       [xp, yp]=ind2sub([nx ny],cen_idx);
 ...
 m_contourf(lon,lat,pmsl); hold on
 m_plot(lon(cen_idx),lat(cen_idx),'rx','linewidth',2)
