...
fignam=[expri1(8:end),'_cloud-',ploterm,'mean_'];
%
...
for ti=hrs 
  ...
  for mi=minu
    ...
    if ~isempty(cloud) 
      ...
      if nti<=3
       plot(cloud.scale,cloud.meandte,'o','MarkerSize',8,'MarkerFaceColor',col(nti,:),'MarkerEdgeColor',[0.1 0.1 0.1],'linewidth',1.3); hold on   
      else
%       plot(cloud.scale(neartopo),cloud.maxdte(neartopo),'^','MarkerSize',8,'MarkerFaceColor',col(nti,:),'MarkerEdgeColor',col(nti,:)); hold on  
       plot(cloud.scale,cloud.meandte,'o','MarkerSize',8,'MarkerFaceColor',col(nti,:),'MarkerEdgeColor',col(nti,:)); hold on   
      end
%       plot(cloud.scale(neartopo),cloud.maxdte(neartopo),'o','MarkerSize',8,'MarkerFaceColor','none','MarkerEdgeColor',[0.1 0.1 0.1],'linewidth',1.3); 
      ntii=ntii+1;
      lgnd{ntii}=[num2str(mod(hr+9,24),'%2.2d'),s_min,' LT']; 
      %----
...
% ylabel(['Mean of ',ploterm,' ( J kg^-^1)']);
ylabel({'(Mean of first 10 maximum)','MDTE (J kg^-^1)'});
