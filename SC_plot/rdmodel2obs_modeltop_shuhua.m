...
case(1)
   ...
   %dir='/SAS003/scyang/WRFEXPSV3/2008IOP8/new/e18_1deg_upper/innov/all_levs/';
   %dir='/SAS003/scyang/WRFEXPSV3/2008IOP8/new/e18_1deg_upper/innov/all_levs_bdarm15/';
   dir='/SAS003/scyang/WRFEXPSV3/2008IOP8/modeltop_test/e18_1deg_upper/modeltoplevel_replaced/';
   ll='-';
end
mon=6;
dd=14;
...
file=['model2obs_direct__2008-06-',num2str(dd),'_',num2str(hh,'%2.2d'),':00:00'];
infile=[dir,file]
    ...
    for j=1:obs{i,n}.nobs
        ...
        if(j>1 & i==1)
           if( abs(obs{i,n}.yb1(j)) <=1.e-4 & abs(obs{i,n}.yb1(j-1)) >1.e-3)
               ...
               ro_diffs1km(iprf)=((y(jprf,iz1)-yb1(jprf,iz1))/y(jprf,iz1)).^2;
               ro_diffs5km(iprf)=((y(jprf,iz5)-yb1(jprf,iz5))/y(jprf,iz5)).^2;
               ro_topdiff1km(iprf)=abs(ro_yo(iprf) -ro_yb(iprf) )/y(jprf,iz1);
               ro_topdiff5km(iprf)=abs(ro_yo(iprf) -ro_yb(iprf) )/y(jprf,iz5);
               ro_topdiffs1km(iprf)=((ro_yo(iprf) -ro_yb(iprf) )/y(jprf,iz1)).^2;
               ro_topdiffs5km(iprf)=((ro_yo(iprf) -ro_yb(iprf) )/y(jprf,iz5)).^2;
           end
        end
