...
switch (nf)
case(1)
   dir='/SAS002/SAS001_Backup/scyang/WRFEXPSV3/2008IOP8/e41/innov/';
   dir1='/SAS002/SAS001_Backup/scyang/WRFEXPSV3/2008IOP8/e41/innov2/';
   ll='-';
   cc='k';
case(2)
   dir='/SAS002/SAS001_Backup/scyang/WRFEXPSV3/2008IOP8/e48b/innov/';
   dir1='/SAS002/SAS001_Backup/scyang/WRFEXPSV3/2008IOP8/e48b/innov2/';
   %dir='/SAS002/SAS001_Backup/scyang/WRFEXPSV3/2008IOP8/e49a/innov3/';
   ll='-';
   cc='b';
case(3)
   dir='/SAS002/SAS001_Backup/scyang/WRFEXPSV3/2008IOP8/e49a/innov/';
   dir1='/SAS002/SAS001_Backup/scyang/WRFEXPSV3/2008IOP8/e49a/innov2/';
   ll='-';
   cc='r';
end
mon=6;
...
for i=1:4
    ...
    for j=1:obs{i,n}.nobs
        ...
        figure(3)
        if(n<=nt)
        m_plot(obs{i,n}.lon(j),obs{i,n}.lat(j),'b.','markersize',20);
        else
        m_plot(obs{i,n}.lon(j),obs{i,n}.lat(j),'r.','markersize',20);
        end
...
for n=1:nt
    ...
    for j=1:obs{nvar,n}.nobs 
        ...
        biasi(ncount(nz),nz)=obs{nvar,n}.innov(j);
        rmsi(ncount(nz),nz)=obs{nvar,n}.innov(j).^2;
        biasr(ncount(nz),nz)=obs{nvar,n}.resid(j);
        rmsr(ncount(nz),nz)=obs{nvar,n}.resid(j).^2;
        end
    end
...
if(nf==1)
   ...
   for nz=1:9
       [hbiasi(nz,nvar,nf-1),pbiasi(nz,nvar,nf-1)]=ttest(biasi1(1:ncount(nz),nz),biasi(1:ncount(nz),nz),0.3,'both');
       [hrmsi(nz,nvar,nf-1),prmsi(nz,nvar,nf-1)]=ttest(rmsi1(1:ncount(nz),nz),rmsi(1:ncount(nz),nz),0.3,'both');
       [hbiasr(nz,nvar,nf-1),pbiasr(nz,nvar,nf-1)]=ttest(biasr1(1:ncount(nz),nz),biasr(1:ncount(nz),nz),0.3,'both');
       [hrmsr(nz,nvar,nf-1),prmsr(nz,nvar,nf-1)]=ttest(rmsr1(1:ncount(nz),nz),rmsr(1:ncount(nz),nz),0.3,'both');
   end
end
...
y0=3;
 ...
 set(gca,'ylim',[-1005 -690],'xlim',[-1. 3],'yticklabel',{'1000','900','800','700','600','500'})
    if(nf>1)
       for nz=1:9
           msize=20*(1+(3-nf)*0.5);
           if(hbiasi(nz,nvar,nf-1)>0)
              plot(innov(nz,1)./ncount(nz),-z(nz),'marker','.','markersize',msize,'color',cc);hold on
           end
           if(hrmsi(nz,nvar,nf-1)>0)
              plot(sqrt(innov(nz,2)./ncount(nz)),-z(nz),'marker','.','markersize',msize,'color',cc);hold on
           end
       end 
    end
 end
 ...
 if(nf>1)
        for nz=1:9
            %msize=20*(1+(3-nf)*0.5);
            msize=20;
            if(hbiasr(nz,nvar,nf-1)>0)
               plot(resid(nz,1)./ncount(nz),-z(nz),'marker','.','markersize',msize,'color',cc);hold on
            end
            if(hrmsr(nz,nvar,nf-1)>0)
               plot(sqrt(resid(nz,2)./ncount(nz)),-z(nz),'marker','.','markersize',msize,'color',cc);hold on
            end
        end
 end

 end
 set(gca,'ylim',[-1005 -690],'xlim',[-1. 3.5],'ytick',[-1000:100:-500],'yticklabel',{'1000','900','800','700','600','500'})
...
if(nf==1)
title([subtit{nvar},' Bias/RMS of Obs-Analysis for ',cvar],'fontweight','bold')
xlabel('Error (m/s)')
ylabel('Pressure (hPa)')
