...
L=[0.1 0.5 1 1.5 2 2.5 3 3.5 4 4.5 5 5.5];
%---set
addpath('/work/pwin/m_map1.4/')
addpath('/work/pwin/data/colorbar/')
load '/work/pwin/data/colormap_qr2.mat';  cmap(2:13,:)=colormap_qr2([1 3 5 8 9 10 11 13 14 15 16 17],:); cmap(1,:)=[1 1 1];
%---
varinam='total qc';    filenam=[expri,'_TPW-qc_'];  
...
%------read netcdf data--------
   ...
   varid  =netcdf.inqVarID(ncid,'QCLOUD');   qc  =double(netcdf.getVar(ncid,varid));
   varid  =netcdf.inqVarID(ncid,'P');       p   =netcdf.getVar(ncid,varid);
   varid  =netcdf.inqVarID(ncid,'PB');      pb  =netcdf.getVar(ncid,varid); 
%---integrate TPW---
   P=(pb+p);   [nx ny nz]=size(qc);
   dP=P(:,:,1:nz-1)-P(:,:,2:nz);    
   tpw= dP.*( (qc(:,:,2:nz)+qc(:,:,1:nz-1)).*0.5 ) ;    
   TPW=sum(tpw,3)./g;
%---read wind varaibles---------------
   if wid~=0
