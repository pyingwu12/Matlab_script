%function cov_obsmodel_prof(vonam,vmnam,memsize,pradar,pela,paza,pdis)
%-------------------------------------------
%  Cov.(obs_point,model_profile) 
%-------------------------------------------
clear

hr=0;  minu='00';   vonam='Vr';  vmnam='T';  memsize=40;

%expri='e01';     infilenam='fcst';    dirnam='cycle';  type=infilenam;
expri='largens';  infilenam='wrfout';  dirnam='pert';   type='wrfo';
pradar=4;  pela=6;  paza=178;  pdis=98;  
%pradar=3;  pela=6;  paza=298;  pdis=88;  

len=1.5;     slopx=1;   slopy=0; %integer 

hclr=36/111;  vclr=12000; vclr2=4000; 
...
switch vmnam(1)
case char('Q')
  if strcmp(vonam,'Zh')==1
   L=[-3 -2.5 -2 -1.5  -1 -0.5  0.5 1 1.5 2 2.5 3];  %Zh-QXXX
  else
   L=[-1.1 -.9 -.7 -.5  -.3 -0.1  0.1 .3 .5 .7 .9 1.1];  %Vr-QXXX
  end
case char('T')
  if strcmp(vonam,'Zh')==1;
   L=[-11 -9 -7 -5 -3 -1  1 3 5 7 9 11];  %Zh-T
  else
   L=[-3 -2.5 -2 -1.5  -1 -0.5  0.5 1 1.5 2 2.5 3]; %Vr-T
  end
otherwise
  L=[-30 -25 -20 -15 -10 -5  5 10 15 20 25 30]; %Vr/Zh-U,V
end
%---
if strcmp(vmnam(1),'Q')==1;  s_vm=[vmnam(1),lower(vmnam(2))];  else  s_vm=vmnam; end
corrvari=[vonam,'-',lower(s_vm)];  %
varinam=['Cov.',corrvari];    filenam=[expri,'-cov_']; 
%
...
%---read model variable---
      ...
      for i=1:np
         ...
         if     strcmp(vmnam(1),'Q')==1;   vm=vm*1000;
...
%---background error correlation calculation---
   ...
   Ae=A-At;                             be=b-bt;
   %---covariance
   cov0=(Ae*be)./(memsize-1); 
   cov1=reshape(cov0,nzi,np);
   plotvar=cov1;
...
%---plot---
 ...
 %}
   ...
   %[c hp]=contourf(xi,zi,plotvar);   set(hp,'linestyle','none');  hold on;  
