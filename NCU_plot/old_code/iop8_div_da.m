%function iop8_div_da(hm,expri,plothgt)
...
clear;  
hm='00:00';   expri='true';  plothgt=4000;  %lev=4; 
...
%---read netcdf data and calculate the variable--------
  ...
  %---interpolation to 1km-height
    ...
    for i=1:nx
      for j=1:ny
      X=squeeze(zg(i,j,:));
      Y=squeeze(div(i,j,:));     variso(i,j)=interp1(X,Y,hgt.iso(i,j),'linear');
...
%------plot---=================================
   ...
   %
   m_contour(x,y,wiso,[0.1 2],'-k','LineWidth',1); 
   m_contour(x,y,wiso,[-0.1 -2],'--k','LineWidth',1); 
   %
