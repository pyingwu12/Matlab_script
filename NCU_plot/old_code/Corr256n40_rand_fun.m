function [corr256 corr40 p_lon p_lat zhmean]=Corr256n40_rand_fun(hr,minu,vonam,vmnam,pradar,pela,paza,pdis,sub)
%-------------------------------------------------
%   Corr.(obs_point,model_2D-level(subdomain)) 
%-------------------------------------------------
%clear
%hr=0;  minu='00';   vonam='Zh';  vmnam='QVAPOR';
%pradar=4;  pela=0.5;  paza=248;  pdis=83;     sub=36;
...
%---estimate background error---
% A:2-D, 3-dimention array, b:point, vector
A=reshape(vmen(:,:,randmem),sub2*sub2,memsize);       b=voen(randmem);
at=mean(A,2);                        bt=mean(b);  %estimate true
At=repmat(at,1,memsize);
Ae=A-At;                             be=b-bt;
%---variance---
