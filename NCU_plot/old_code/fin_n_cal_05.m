function  fin_n_cal_05(expri,memsize)
...
%=====================   
   ...
   per=abs(vr./spe);
   fin_vr=find(per<=0.005 & rad==3 & ela<4);  svr=vr(fin_vr); saza=aza(fin_vr);
...
%------model space----      
      ...
      for i=1:ng2
        if (px-ng+i-1-1>=1 & px-ng+i-1-1<=nx & py+ng-i+1-1>=1 & py+ng-i+1-1<=330)
