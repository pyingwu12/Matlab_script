...
for i=1:length(aza)
  ...
  for j=1:vn
    maxcorr=max(max(abs(corr40{j}),abs(corr256{j})));  err(n,j)=sum(abs(corr40{j}-corr256{j}))/length(corr256{j})/maxcorr;
  end
end

s_rad=num2str(rad); s_ela=num2str(ela); s_dis=num2str(dis); s_sub=num2str(sub);
save(['stnerr_',s_rad,'-',s_ela,'-',s_dis,'_',s_sub,'.mat'],'err','lon','lat')
