function [corr256 corr40 p_lon p_lat]= Corr256n40allvari_fun(hr,minu,pradar,pela,paza,pdis,sub)
%-------------------------------------------------
...
%---difine px, py for th line---
   ...
   vm=netcdf.getVar(ncid,varid,[px-sub-1 py-sub-1 lev-1 0],[sub2+1 sub2+1 1 1]); vm=squeeze(vm);
   QRAIN(:,:,mi)=vm(1:sub2,1:sub2)*1000;

   varid  =netcdf.inqVarID(ncid,'QVAPOR');
   vm=netcdf.getVar(ncid,varid,[px-sub-1 py-sub-1 lev-1 0],[sub2+1 sub2+1 1 1]); vm=squeeze(vm);
   QVAPOR(:,:,mi)=vm(1:sub2,1:sub2)*1000;

   varid  =netcdf.inqVarID(ncid,'T');
   vm=netcdf.getVar(ncid,varid,[px-sub-1 py-sub-1 lev-1 0],[sub2+1 sub2+1 1 1]); vm=squeeze(vm);
   T(:,:,mi)=vm(1:sub2,1:sub2);

   varid  =netcdf.inqVarID(ncid,'QCLOUD');
   vm=netcdf.getVar(ncid,varid,[px-sub-1 py-sub-1 lev-1 0],[sub2+1 sub2+1 1 1]); vm=squeeze(vm);
   QCLOUD(:,:,mi)=vm(1:sub2,1:sub2)*1000;

   netcdf.close(ncid);
   fclose(fid);
end   %member

vmnam={'V';'U';'QVAPOR';'QRAIN';'T';'QCLOUD'};
vonam={'Vr';'Zh'};
%}
m=0;
for j=1:length(vonam)
  ...
  for i=1:length(vmnam)
    ...
    %---estimate background error---
    % A:2-D, 3-dimention array, b:point, vector
    eval(['A=reshape(',vmen,'(:,:,1:memsize),sub2*sub2,memsize);']);       eval(['b=',voen,'(1:memsize);'])
    at=mean(A,2);                        bt=mean(b);  %estimate true
    At=repmat(at,1,memsize);
    Ae=A-At;                             be=b-bt;
    %---variance---
