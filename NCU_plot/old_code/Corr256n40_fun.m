function [corr256 corr40 p_lon p_lat zhmean]=Corr256n40_fun(hr,minu,vonam,vmnam,pradar,pela,paza,pdis,sub)
%-------------------------------------------------
%   Corr.(obs_point,model_2D-level(subdomain)) 
%-------------------------------------------------
%clear
%hr=0;  minu='00';   vonam='Zh';  vmnam='QCLOUD';
%pradar=3;  pela=0.5;  paza=263;  pdis=113;     sub=12;
...
%---difine px, py for th line---
   ...
   vm=squeeze(vm);
   if     strcmp(vmnam(1),'Q')==1;   vmen(:,:,mi)=vm(1:sub2,1:sub2)*1000;        
...
%---correlation--- 
corr256=cov./(varae.^0.5)./(varbe^0.5);
%==========================================================
%randmem=randi(memsize,1,memsizes);
memsize=memsizes;
clear A b at bt At Ae be varae varbe cov
%---estimate background error---
% A:2-D, 3-dimention array, b:point, vector
A=reshape(vmen(:,:,1:memsize),sub2*sub2,memsize);       b=voen(1:memsize);
at=mean(A,2);                        bt=mean(b);  %estimate true
At=repmat(at,1,memsize);
Ae=A-At;                             be=b-bt;
%---variance---
...
cov=(Ae*be)./(memsize-1); %covariance
%---correlation---
corr40=cov./(varae.^0.5)./(varbe^0.5);
