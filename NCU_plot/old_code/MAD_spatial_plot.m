function  MAD_spatial_plot(plotid)

%clear
hr=0;  minu='00';  

%plotid=11;  %!!1:Vr-V, 2:Vr-U, 3:Vr-qv, 4:Vr-qr, 5:Zh-V, 6:Zh-U....
...
%
varinam=['MADs of CORR(',vonam,', ',s_vm,')'];    filenam=['Corr-MAD_'];
%
sub=12;
%sub2=sub*2+1;
...
dom='02'; year='2008'; mon='06'; date='16';
...
s_rad=num2str(irad);
%
load(['MAD_',s_rad,'-0.5-53_12.mat']);  Var=err(1:2:end,plotid);
lon=lon(1:2:end); lat=lat(1:2:end);
plot_point(Var,lon,lat,cmap,L);

load(['MAD_',s_rad,'-0.5-63_12.mat']);  Var=err(1:2:end,plotid);
lon=lon(1:2:end); lat=lat(1:2:end);
plot_point(Var,lon,lat,cmap,L);

load(['MAD_',s_rad,'-0.5-73_12.mat']);  Var=err(:,plotid);
plot_point(Var,lon,lat,cmap,L);
%
%load(['stnerr_',s_rad,'-0.5-68_12.mat']);  Var=err(:,plotid); 
%hp=plot_point(Var,lon,lat,cmap,L);
%
load(['MAD_',s_rad,'-0.5-83_12.mat']);  Var=err(:,plotid);
hp=plot_point(Var,lon,lat,cmap,L);

%load(['MAD_',s_rad,'-0.5-78_12.mat']);  Var=err(:,plotid);
%hp=plot_point(Var,lon,lat,cmap,L);
%
load(['MAD_',s_rad,'-0.5-93_12.mat']);  Var=err(:,plotid);
plot_point(Var,lon,lat,cmap,L);
%
load(['MAD_',s_rad,'-0.5-103_12.mat']);  Var=err(:,plotid);
hp=plot_point(Var,lon,lat,cmap,L);

load(['MAD_',s_rad,'-0.5-113_12.mat']);  Var=err(:,plotid);
plot_point(Var,lon,lat,cmap,L);
