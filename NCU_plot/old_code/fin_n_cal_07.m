function  fin_n_cal_07(expri,memsize)
...
%=====================   
   infile=['/SAS011/pwin/201work/data/wrf2obs_uv/largens_wrf2obs_e01/fcstmean','_d',dom,'_',year,'-',mon,'-',date,'_',s_hr,':',minu,':00'];
   A=importdata(infile); rad=A(:,1); ela=A(:,2); aza=A(:,3);  vr=A(:,8); 
   sou=A(:,10);  sov=A(:,11);   
   spe=sqrt(sou.^2+sov.^2);
   per=abs(vr./spe);
   fin_vr=find(per<=0.01 & rad==3 & ela<4 & aza>300 & aza<330);  svr=vr(fin_vr); saza=aza(fin_vr);
