function TPW_meanmember(hr,acch,expri,expid)

%clear
%hr=20; expri='DA1517vrzh'; expid=2;
...
%----set---- 
vari='TPW';   filenam=[expri,'_TPW-Mmb_'];  type='member';  
if expid==1; indir=['/SAS004/pwin/wrfout_morakot_',expri];
elseif expid==2; indir=['/SAS002/pwin/expri_241/morakot_en_',expri];
elseif expid==3; indir=['/SAS002/pwin/expri_241/morakot_',expri]; 
end
...
%---set filename---    
   ...
   [mem]=mean_member(ti,acch,expri);

   mi=mem;  
     nen=num2str(mi);
        if mi<=9
         if expid==1; infile=[indir,'/wrfout_d03_2009-08-',s_date,'_',s_hr,':00:00_0',nen];
         elseif expid==2; infile=[indir,'/pert0',nen,'/wrfout_d03_2009-08-',s_date,'_',s_hr,':00:00'];
         elseif expid==3; infile=[indir,'/cycle0',nen,'/anal_d03_2009-08-',s_date,'_',s_hr,':00:00'];
         end
        else
         if expid==1; infile=[indir,'/wrfout_d03_2009-08-',s_date,'_',s_hr,':00:00_',nen];
         elseif expid==2; infile=[indir,'/pert',nen,'/wrfout_d03_2009-08-',s_date,'_',s_hr,':00:00'];  
         elseif expid==3; infile=[indir,'/cycle',nen,'/anal_d03_2009-08-',s_date,'_',s_hr,':00:00'];
         end
        end
%----read netcdf data--------
