...
expri='e02';   sth=0; hrs=24;  date=25;  vari='QVAPOR';
hei=50:50:5000;    filenam=[expri,'_',vari,'-sprd_point_low']; 
%hei=100:200:11000; filenam=[expri,'_',vari,'-sprd_point']; 
%----set---- 
varit=[vari,' at Douliou'];   %filenam=[expri,'_T-sprd_point']; 
...
%====
   for mi=1:40 
     ...
     varid  =netcdf.inqVarID(ncid,vari);     qx.ori=(netcdf.getVar(ncid,varid)).*1000;      
     ...
     qx.point=double(qx.ori(xp,yp,:));                                       %%% T above P(xp,yp)
     qx.ens(:,mi)=interp1(squeeze(zg),squeeze(qx.point),hei,'linear');  %%% interpolation to isoheight
%---
     netcdf.close(ncid)
   end %member
   %
   %sprd(:,ni)=spread(tp);  zgt(:,ni)=mean(zg,2);
   sprd(:,ni)=spread(qx.ens);   %%% T spread above P(xp,yp), ni changes with time
...
tit=[expri,'  ',varit,'  ',s_mon,'/',num2str(date),'  (',num2str(lon(xp,yp)),'°E , ',num2str(lat(xp,yp)),'°N)'];
