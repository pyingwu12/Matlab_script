...
titnam='Upward surface Qv flux';   fignam=['QFX_',exptext,'_'];

acci=zeros(nexp,lenh);
for i=1:nexp
   qfx(i,:)=cal_qfx(expri{i},time,sth,lenh,dom{i},bdy,typst,ccc);
   disp([expri{i},' done'])
end
...
for i=1:nexp
plot(qfx(i,:),'color',cexp(i,:),'Linestyle',lexp{i},'LineWidth',2.2); hold on
end
%
...
xlabel('Time (JST)','fontsize',18);  ylabel('Moisture Flux (kg m^-^2 s^-^1)','fontsize',18)
