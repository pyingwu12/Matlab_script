...
for i=1:length(lat)          
    ...
    if value(i)<=colL(1)
      ...
      m_plot(lon(i),lat(i),Marker,'color',cmap(1,:),'Markerfacecolor',cmap(1,:),'MarkerSize',Msize);  
      
    elseif value(i)>colL(end)
        
      m_plot(lon(i),lat(i),Marker,'color',cmap(end,:),'Markerfacecolor',cmap(end,:),'MarkerSize',Msize);   
      
    else
        
      for ci=1:length(colL)-1
        if value(i)>colL(ci) && value(i)<=colL(ci+1)
          m_plot(lon(i),lat(i),Marker,'color',cmap(ci+1,:),'Markerfacecolor',cmap(ci+1,:),'MarkerSize',Msize);
        end
      end
