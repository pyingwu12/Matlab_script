...
%
titnam='10-m Wind speed Spread';   fignam=['WindFiltSprdTs_',outag,'_'];
%
...
for iexp=1:nexp
  for itime=1:ntime
    ...
    for imem=1:pltensize 
     ...
     u=ncread(infile,'u10m',[1 1 pltime(itime)],[Inf Inf 1],[1 1 1]); 
     v=ncread(infile,'v10m',[1 1 pltime(itime)],[Inf Inf 1],[1 1 1]); 
     vari0(:,:,imem) = (u.^2+v.^2).^0.5;
    end

    tmp=var(vari0(1+idifx:end-idifx,1+idifx:end-idifx,:),0,3);
    sprdall(itime,iexp)=sqrt(mean(tmp(:)));
    
    %---filtering---
    pert=vari0-repmat(mean(vari0,3),1,1,pltensize,1);
    for imem=1:pltensize  
