...
for imem=1:pltensize 
  infile=[indir,'/',num2str(member1(imem),'%.4d'),'/',infilename,'.nc'];   
  if imem==1
    lon = double(ncread(infile,'lon'));    lat = double(ncread(infile,'lat'));
    data_time = (ncread(infile,'time'));   
    [nx, ny]=size(lon); ntime=length(data_time);
    vari0_diff=zeros(nx,ny,pltensize,ntime);
    vari0_same=zeros(nx,ny,pltensize,ntime);
  end  
  vari0_diff(:,:,imem,:) = ncread(infile,'pmsl');
end

for imem=1:pltensize 
  infile=[indir,'/',num2str(member2(imem),'%.4d'),'/',infilename,'.nc'];    
  vari0_same(:,:,imem,:) = ncread(infile,'pmsl');
end
...
[nx1, ny1, ~, ~]=size(vari1_same);

%   nrm_var_same= var(vari1_same,0,3)./ mean(vari1_same,3) ./ mean(vari1_same,3);  
%   nrm_sprd_same = ( sum( nrm_var_same,[1 2]) /nx1/ny1).^0.5 *1000;  
%   nrm_var_diff= var(vari1_diff,0,3)./ mean(vari1_diff,3) ./ mean(vari1_diff,3);  
%   nrm_sprd_diff = ( sum( nrm_var_diff,[1 2]) /nx1/ny1).^0.5 *1000;
  
  var_same= var(vari1_same,0,3);  
  sprd_same = ( sum( var_same,[1 2]) /nx1/ny1).^0.5;
  var_diff= var(vari1_diff,0,3);  
