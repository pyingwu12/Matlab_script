...
clear;   ccc=':';
%---setting
expri='TWIN024B'; s_date='22';  hr=23;  minu=[20]; 
%---
...
%       den_prof(:,i)=interp1(squeeze(zg(i,100,:)),squeeze(dencity(i,100,:)),zgi,'linear');   
      w1_prof(:,i)=interp1(squeeze(zg0(i,yp,:)),squeeze(w.stag(i,yp,:)),zgi,'linear');   
      w2_prof(:,i)=interp1(squeeze(zg0(i,yp,:)),squeeze(w.stag2(i,yp,:)),zgi,'linear');   
   end
...
%
    ...
    [~, hp]=contourf(xi2,zi2,dwdt*1e4,20,'linestyle','none'); colorbar; caxis([-20 20])
