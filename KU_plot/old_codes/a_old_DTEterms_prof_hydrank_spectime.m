...
s_date='22';  hr=23;  minu=[30 40 50];
%---
rk_date='23'; rk_hr='00'; rk_min='00';
%---
year='2018'; mon='06'; 
infilenam='wrfout';  dom='01'; 
%---
indir='/mnt/HDD123/pwin/Experiments/expri_twin';  outdir=['/mnt/e/figures/expri_twin/',expri(1:7)];
titnam='DTE terms';   fignam=[expri1(8:end),'_DTEterms_rk',rk_hr,rk_min,'_'];
%---
g=9.81;
...
for ti=hr
  ...
  for mi=minu   
    ...
    %----------------------------------
    rk_infile=[indir,'/',expri2,'/',infilenam,'_d',dom,'_',year,'-',mon,'-',rk_date,'_',rk_hr,ccc,rk_min,ccc,'00'];
     qr = ncread(rk_infile,'QRAIN');  qr=double(qr);   
     qs = ncread(rk_infile,'QSNOW');  qs=double(qs);    
     qg = ncread(rk_infile,'QGRAUP'); qg=double(qg);
     qi = ncread(rk_infile,'QICE');   qi=double(qi);
     qc = ncread(rk_infile,'QCLOUD'); qc=double(qc);
     rk_hyd=(qr+qs+qg+qi+qc)*1e3; 
    [nx, ny, nz]=size(rk_hyd);
    hyd2D=sum(rk_hyd,3);
...
%-------  
    ...
    xlabel(['Rank of vertical accumulated hydrometeor at ',rk_hr,rk_min,' LT']); ylabel('Height (km)')
