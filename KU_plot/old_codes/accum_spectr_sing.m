...
close all
%---setting
expri='TWIN001B';  stday=21;  sths=[15 18 21 24 27 30 34 37]; acch=1; 
%--
...
col=col0(1:2:end,:);
%
titnam='Rainfall spectra';   fignam=[expri,'_accum-sp_'];
%---
lenhr=length(sths);
%
%---
...
for ti=1:lenhr     
  ...
  %---calculate Kh from 2D wave numbers---
    ...
    nk2=round(nk); 
    KE.kh=zeros(max(max(nk2)),lenhr); 
  end  %if ti==1
  %
  %---2D fft 
  rain.fft=fft2(rain.ori);    
  %---calculate KE (power of the FFT)---
  KE.twoD = (abs(rain.fft).^2)/nx/ny ;  %2-D    
  %--shift
