function modte=cal_DTEmo_twin(indir,expri1,expri2,ymd,sth,lenh,minu,ccc)

% calculate different total energy (DTE, Zhang 2007?) with considering laten heat)
% defined as "moist DTE"=1/2(u_diff^2+v_diff^2) + cp/Tr * T_diff^2 + Lv * qv^2
...
for ti=1:lenh    
  ...
  for tmi=minu
    ...
    modte{nti}=1/2*(u.diff.^2+v.diff.^2) + cp/Tr*t.diff.^2 + Lv*s.diff.^2;
  end %tmi
