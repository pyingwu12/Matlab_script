...
close all
%---setting
expri='TWIN018';  expri1=[expri,'Pr001qv062221'];  expri2=[expri,'B']; 
stday=22;  hrs=[21 22 23 24 25 26 27 28];  lev=1:33;    minu=[00 20 40];
%--
...
lenh=length(hrs);
%----
lgnd=cell(lenh*length(minu),1);   nti=0;
for ti=1:lenh
  hr=hrs(ti);
...
outfile=[outdir,'/',fignam,'d',dom,'_',mon,num2str(stday),'_',s_sth,s_edh,s_min,...
    '_',num2str(lenh),'step_lev',num2str(lev(1),'%.2d'),num2str(lev(end),'%.2d')];
