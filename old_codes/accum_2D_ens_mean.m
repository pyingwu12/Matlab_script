...
%---read ensemble data---
   for mi=1:ensize
      ...
      rain{mi}=double(rc{2}-rc{1}+rnc{2}-rnc{1}+rsh{2}-rsh{1});
   end %mi
   hgt = ncread(infile,'HGT'); 
   %---caculate mean---
   [meanrain, PM, PMmod]=cal_PM(rain);  
   %
   %---plot---  
   for pi=plotid   % 1:mean, 2:PM, 3:PMmod
     switch (pi); case 1; plotvar=meanrain';  case 2; plotvar=PM'; case 3; plotvar=PMmod';  end          
     ...
     xlabel('(km)'); ylabel('(km)');  
     tit={[expri,'  (',typst{pi},')'];[titnam,'  ',s_sth,s_min,'-',s_edh,s_min,' UTC']};
