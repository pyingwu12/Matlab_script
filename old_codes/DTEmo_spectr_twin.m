  ...
  %---calculate---
   ...
   %---
    ...
    %---calculate KE (power of the FFT)---
       KE.twoD(:,:,li) = (abs(u.fft).^2 + abs(v.fft).^2 + cp/Tr*abs(t.fft).^2 + Lv^2/cp/Tr*abs(qv.fft).^2 + R*Tr*abs(psfc.fft/Pr).^2)/nx/ny ;  %2-D low level mean      
       KEp.twoD(:,:,li) = (abs(u.perfft).^2 + abs(v.perfft).^2 + cp/Tr*abs(t.perfft).^2 + Lv^2/cp/Tr*abs(qv.perfft).^2 + R*Tr*abs(psfc.perfft/Pr).^2)/nx/ny ;          
    end %li=lev
    %--shift
