...
%---experiments
% expri1={'TWIN001Pr001qv062221';'TWIN003Pr001qv062221';'TWIN013Pr001qv062221'};   exptext='exp1313qva1510';
% expri2={'TWIN001B';'TWIN003B';'TWIN013B'};
% expnam={'FLAT';'TOPO';'loTOPO'};
...
expri1={'TWIN001B2'};   exptext='TWIN001B2_3';
expri2={'TWIN001B3'};
expnam={'TWIN001B2'};
col=[0,0.447,0.741];
...
%
%---------------------------------------------------
DTE_dm=zeros(nexp,ntime); 
if plotarea~=0; DTE_am=zeros(nexp,ntime,narea); end
...
for ei=1:nexp  
  ...
  for ti=1:lenh    
    ...
    for tmi=minu
      ...
      DTE_dm(ei,nti) = mean(mean(MDTE));  % domain mean of 2D DTE      
      if plotarea~=0  % area mean of 2D DTE
        for ai=1:narea
        DTE_am(ei,nti,ai) = mean(mean( MDTE(xarea(ai,:),yarea(ai,:)) ));
        end
      end      
    end %minu
