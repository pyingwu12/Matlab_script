...
titnam='Diff. laten heat (area mean of vertical weighted average)';   fignam=['MDiffLH_',exptext,'_'];
...
%---------------------------------------------------
DiffLH_dm=zeros(nexp,ntime); 
if plotarea~=0; DiffLH_am=zeros(nexp,ntime,narea); end
...
for ei=1:nexp  
  ...
  for ti=1:lenh    
    ...
    for tmi=minu
      ...
      DiffLH_dm(ei,nti) = mean(mean(MDiffLH));  % domain mean   
      if plotarea~=0  % area mean
        for ai=1:narea
        DiffLH_am(ei,nti,ai) = mean(mean( MDiffLH(xarea(ai,:),yarea(ai,:)) ));
        end
      end      
    end %minu
...
for ei=1:nexp
  plot(DiffLH_dm(ei,:),'LineWidth',2.5,'color',col(ei,:)); hold on
  if plotarea~=0
  for ai=1:narea
    plot(DiffLH_am(ei,:,ai),'LineWidth',2.2,'color',col(ei,:),'linestyle',linestyl{ai},...
