...
titnam='Diff. thermal heat (area mean of vertical weighted average)';   fignam=['MDiffTE_',exptext,'_'];
...
%---------------------------------------------------
DiffTE_dm=zeros(nexp,ntime); 
if plotarea~=0; DiffTE_am=zeros(nexp,ntime,narea); end
...
for ei=1:nexp  
  ...
  for ti=1:lenh    
    ...
    for tmi=minu
      ...
      %---
      MDiffTE=cal_DiffThE_2D(infile1,infile2);      
      DiffTE_dm(ei,nti) = mean(mean(MDiffTE));  % domain mean   
      if plotarea~=0  % area mean
        for ai=1:narea
        DiffTE_am(ei,nti,ai) = mean(mean( MDiffTE(xarea(ai,:),yarea(ai,:)) ));
        end
      end      
    end %minu
...
for ei=1:nexp
  plot(DiffTE_dm(ei,:),'LineWidth',2.5,'color',col(ei,:)); hold on
  if plotarea~=0
  for ai=1:narea
    plot(DiffTE_am(ei,:,ai),'LineWidth',2.2,'color',col(ei,:),'linestyle',linestyl{ai},...
