...
close all
%---setting
expri='ens11';  stday=21;  hrs=[15 18 21 24 27 30 34 37]; ensize=3;  lev=1:17;  
%--
...
%
titnam='KE spectra';   fignam=[expri,'_KEens-sp_'];
%---
lenhr=length(hrs);
%
%----
...
for ti=1:lenhr
  ...
  %---calculate Kh from 2D wave numbers---
    ...
    KEp.khm=zeros(max(max(nk2)),lenhr);
  end %if ti==1
  KE.kh=zeros(max(max(nk2)),ensize); 
  KEp.kh=zeros(max(max(nk2)),ensize);
  %---members---   
  for mi=1:ensize
    nen=num2str(mi,'%.2d');
...
%---perturbation---
for ti=1:lenhr
plot(KEp.khm(:,ti),'LineWidth',2,'LineStyle','--','color',col(ti,:))
end
