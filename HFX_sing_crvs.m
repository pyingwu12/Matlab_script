...
titnam='Upward Heat flux at surface';   fignam=['HFX_',exptext,'_'];

acci=zeros(nexp,lenh);
for i=1:nexp
   hfx(i,:)=cal_hfx(expri{i},time,sth,lenh,dom{i},bdy,typst,ccc);
   disp([expri{i},' done'])
end
...
for i=1:nexp
plot(hfx(i,:),'color',cexp(i,:),'Linestyle',lexp{i},'LineWidth',2.2); hold on
end
%
...
xlabel('Time (JST)','fontsize',18);  ylabel('Heat Flux (W m^-^2)','fontsize',18)
