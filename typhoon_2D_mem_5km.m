...
pltime=[17 20];   accuh=3; 
member=1; % member=[41 2];  
...
% expnam='Hagibis01kme05'; infilename='201910111800';%hagibis
expnam='e02nh01G'; infilename='201910111800';%hagibis
...
%
% titnam='10-m Wind speed';   fignam0=[expnam,'_typhoon-wind_'];  unit='m s^-^1';
...
% plotid='wind';
%
titnam='rainfall';   fignam0=[expnam,'_typhoon-rain_'];  unit='mm';
...
for mem=member
  ...
  infile= [indir,'/',s_mem,'/',infilename,'.nc'];
  %---read---
...
%---plot
      ...
      tit={[expnam,'  ',titnam];[datestr(pltdate,'mm/dd HHMM'),'  (',num2str(ai),'-h rain, mem ',num2str(mem,'%.4d'),')']};
          case('rain')
      m_contour(lon,lat,squeeze(spd10(:,:,ti)),[25 25],'color',[0.8 0.4 0.5],'linewidth',2.8);
      tit={[expnam,'  ',num2str(ai),'-h ',titnam];[datestr(pltdate,'mm/dd HHMM'),'  (25 m/s, mem ',num2str(mem,'%.4d'),')']};
      end
