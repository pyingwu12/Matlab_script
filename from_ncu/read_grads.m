...
n=strcmp('quiet',{varargin{1:2:end}});
if isempty(n)
  l_quiet=0;
else
...
while ~feof(fid_ctl)
  ...
  if line(1)=='*'
    ...
    entry=line(1:(find(line_gaps, 1 )-1));
    words={''};
...
if header.DTYPE          % Gridded data set
  ...
  if exist('grads_name','file')
    [~,header]=grads_name(header,vars);
  end
  
  % reading the bin. file :
  if nargin == 1 || strcmpi(var_name,'all')
    read_data(header);
  elseif nargin == 2
    t_limits=[1 header.TDEF.num];
    ivar=strcmp(var_name,{header.VARS.name});
    if isempty(ivar)
      if ~l_quiet
