%function cov_obsmodel(hr,minu,vonam,vmnam,memsize,pradar,pela,paza,pdis)
%function cov_obsmodel(hr,minu,vonam,vmnam,memsize)
%function cov_obsmodel(pradar,memsize)
%-------------------------------------------------
%   Cov.(obs_point,model_2D-level) 
%-------------------------------------------------
%
clear
%
hr=0;  minu='00';   vonam='Zh';  vmnam='QVAPOR';  memsize=256;
...
%L=[-3 -2.5 -2 -1.5  -1 -0.5  0.5 1 1.5 2 2.5 3];   %Vr-QXXX
%L=[-3 -2.5 -2 -1.5  -1 -0.5  0.5 1 1.5 2 2.5 3]*3; %Vr-U,V
%L=[-3 -2.5 -2 -1.5  -1 -0.5  0.5 1 1.5 2 2.5 3]*20;  %Zh-U,V
L=[-3 -2.5 -2 -1.5  -1 -0.5  0.5 1 1.5 2 2.5 3]*2;  %Zh-QXXX
%---

if strcmp(vmnam(1),'Q')==1;  s_vm=lower(vmnam(1:2));  else  s_vm=vmnam; end
covari=[vonam,'-',lower(s_vm)];  
%
varinam=['Cov.',covari];    filenam=[expri,'-cov_']; 
...
% %---plot---
   ...
   m_plot(lonrad,latrad,'^g','MarkerFaceColor',[0 1 0],'MarkerSize',8)          % radar position
   %
   lev=num2str(lev);  mem=num2str(memsize);
%   tit=[expri,'  ',varinam,'  ',s_hr,'z  (',type,' lev',lev,', mem',mem,')']; 
   tit=['Cov.(',vonam,', ',s_vm,')','  ',mem,' members']; 
   title(tit,'fontsize',15)
   outfile=[outdir,'/',filenam,s_hr,minu,'_',num2str(fin),'_',type,'_',covari,'_lev',lev,'_m',mem];
