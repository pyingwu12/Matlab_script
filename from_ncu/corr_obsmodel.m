...
%function corr_obsmodel(vonam,vmnam,memsize)
function corr_obsmodel(vonam,vmnam,memsize,paza)
%function corr_obsmodel(memsize)
%-------------------------------------------------
...
indir1=['/SAS011/pwin/201work/data/largens_wrf2obs_',expri]; indir2=['/SAS009/pwin/expri_largens/',expri]; 
...
plon=[118.1 121.1]; plat=[20.85 24.1]; %southwest area for paper No.2
...
%---plot---
   ...
   %set(hc,'position',[0.85 0.12 0.025 0.8])
   %
...
%---
   ...
   m_plot(p_lon,p_lat,'xk','MarkerSize',12,'LineWidth',2.8)                                    % obs point
   m_plot(lonrad,latrad,'^','color',[0.1 0.7 0.1],'linewidth',1,'MarkerFaceColor',[0 1 0],'MarkerSize',11)   % radar position
   %---localization radius circle---
   %the=0:0.01:2*pi;  xr=loc*1.12*sin(the)+p_lon;  yr=(loc)*cos(the)+p_lat;
   %m_plot(xr,yr,'color',[0.1 0.1 0.1],'LineStyle','--','LineWidth',0.9);
   %  the=0:0.01:2*pi;  xr=loc*3.17*sin(the)+p_lon;  yr=(loc*3)*cos(the)+p_lat; 
   %  m_plot(xr,yr,'color',[0.1 0.1 0.1],'LineStyle','--','LineWidth',0.9);
   %the=0:0.01:2*pi;  xr=3*loc*3.17*sin(the)+p_lon;  yr=3*(loc*3)*cos(the)+p_lat;
   %m_plot(xr,yr,'color',[0.1 0.1 0.1],'LineStyle','--','LineWidth',0.9);
   %---square for calculte score
   % are=12;
   % m_line([x(px-are,py-are) x(px+are,py-are) x(px+are,py+are) x(px-are,py+are) x(px-are,py-are)], ...
   %           [y(px-are,py-are) y(px+are,py-are) y(px+are,py+are) y(px-are,py+are) y(px-are,py-are)], ...
   %                                               'color',[0.1 0.1 0.1],'LineStyle','--','LineWidth',0.9);
   %---location and height of the obs point
...
%   tit=[expri,'  ',varinam,'  ',s_hr,'z  (',type,' lev',lev,', mem',mem,')']; 
   tit=['CORR(',vonam,', ',s_vm,')','  ',mem,'mem']; 
