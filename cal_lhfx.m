function lhfxall=cal_lhfx(expri,time,sth,lenh,dom,bdy,typst,ccc)
%ccc=':';
...
%---
nti=0;  lhfxall=size(length(pridh),1); 
for ti=pridh
   nti=nti+1;
   hr=ti;
   ...
   if exist(indir,'dir') && ~exist(infile,'file') 
      ...
      switch(typst)
      case('mean')
       lhfxall(nti)=mean(mean(lhfx(bdy+1:end-bdy,bdy+1:end-bdy)));
      case('max')
       lhfxall(nti)=max(max(lhfx(bdy+1:end-bdy,bdy+1:end-bdy)));
      case('min')
       lhfxall(nti)=min(min(lhfx(bdy+1:end-bdy,bdy+1:end-bdy)));
      end %switch   
   end %if exist
end %time