...
saveid=0;
%
pltensize=1000;  hr=[14]; minu=[30];  thresholds=15; 
%
expnam='H01km';
...
%%
    ...
    for thi=thresholds      
      ...
      for i=1:nx
          for j=1:ny
              wind_pro(i,j)=length(find(spd10_ens(i,j,:)>=thi));
          end
      end
...
%       m_contour(lon,lat,mean(spd10_ens,3),[thi thi],'r','linewidth',2); 
      %
      tit={[titnam,' (',num2str(thi),' m/s)'];[month,'/',s_date,' ',s_hr,s_min,'  (',num2str(pltensize),' mem)']};
