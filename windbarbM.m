...
function windbarbM(x,y,U,V,scale,fullbar,color,width)
...
h=line([Xo;XI],[Yo;YI],'linewidth',width,'color',color);
...
idd1=find(check>0);
idd2=spd_tmp(idd1)<=7*ut;
idd=idd1(idd2);
Xtmp(2,idd)=(0.5)*(Xtmp(2,idd)+Xtmp(1,idd));
Ytmp(2,idd)=(0.5)*(Ytmp(2,idd)+Ytmp(1,idd));
line([Xtmp(1,:);Xtmp(2,:)],[Ytmp(1,:);Ytmp(2,:)],'linewidth',width,'color',color);
clear idd idd1 idd2
for k=1:4
    ...
    idd1=find(check>0);
    idd2=spd_tmp(idd1)<=(10*k+7)*ut;
    idd=idd1(idd2);
    Xtmp(2+2*k,idd)=(0.5)*(Xtmp(1+2*k,idd)+Xtmp(2+2*k,idd));
    Ytmp(2+2*k,idd)=(0.5)*(Ytmp(1+2*k,idd)+Ytmp(2+2*k,idd));
    line([Xtmp(1+2*k,:);Xtmp(2+2*k,:)],[Ytmp(1+2*k,:);Ytmp(2+2*k,:)],'linewidth',width,'color',color);
    clear idd idd1 idd2
end
...
patch([Xtmp(3,:);Xtmp(4,:);Xtmp(1,:)],[Ytmp(3,:);Ytmp(4,:);Ytmp(1,:)],tcolor,'Edgecolor',colmap);
for k=2:4
    ...
    Ytmp(2+2*k,idd)=Ytmp(1+2*k,idd);
    idd1=find(check>0);
    idd2=spd_tmp(idd1)<=(30+10*k+7)*ut;
    idd=idd1(idd2);
    Xtmp(2+2*k,idd)=(0.5)*(Xtmp(1+2*k,idd)+Xtmp(2+2*k,idd));
    Ytmp(2+2*k,idd)=(0.5)*(Ytmp(1+2*k,idd)+Ytmp(2+2*k,idd));
    line([Xtmp(1+2*k,:);Xtmp(2+2*k,:)],[Ytmp(1+2*k,:);Ytmp(2+2*k,:)],'linewidth',width,'color',color);
