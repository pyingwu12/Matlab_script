function gaussian_level(expnam,pltensize,pltvari)
% clear
close all
...
titnam=[expnam,'  ',pltvari,'  Diff. from Gaussian'];   fignam=[expnam,'_',pltvari,'-diffGau_'];  
%
...
L=[0 .1 .2 .3 .4 0.5 0.6 0.7 0.8 1 1.1 1.2 1.3 1.4];
...
for imem=1:pltensize     
  ...
  if imem==1
    ...
    vari_ens0=zeros(nx,ny,pltensize,length(data_time));   
  end    
  if isfile(infile)
...
for ti=pltime     
  ...
  for xpi=1:nx
   for ypi=1:ny
     sig=std(squeeze(vari_ens(xpi,ypi,:)),'omitnan');
     ens_me=mean(vari_ens(xpi,ypi,:),'omitnan');
          
     normal_dis=1/(sig*(2*pi)^0.5)*exp(-(1/2)*((x-ens_me)/sig).^2);
     normal_dis(normal_dis+1==1)=0;
