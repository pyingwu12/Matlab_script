...
% expnam='H01MultiE0206'; infilename='201910111800';
expsize=1000; 
%
indir=['/obs262_data01/wu_py/Experiments/',expnam,'/',infilename];
outdir='/home/wu_py/labwind/Result_fig';
if ~isfolder(outdir); outdir='/data8/wu_py/Result_fig'; end
titnam=[expnam,'  Max. wind speed']; fignam0=[expnam,'_wind-max_'];   unit='m/s';
%
...
if mode(ti,5)==0; disp([num2str(ti),' time done']); end
end
spd10_max=squeeze(max(spd10_ens_med,[],3));
%  spd10_max2=squeeze(max(spd10_ens_max,[],3));
if kicksea~=0;   spd10_max(land+1==1)=NaN; spd10_max2(land+1==1)=NaN;  end
%%
%     cmap =[    
...
%% median
%---plot
plotvar=spd10_max; tmp=plotvar(lon>=plon(1) & lon<=plon(2) & lat>=plat(1) & lat<=plat(2) );
...
%
%m_contour(lon,lat,land,[0.2 0.2],'linewidth',1.2,'color',[0.1 0.1 0.1],'linestyle','--') %--coast line by mfhm
% m_coast('color','k');
...
% 
tit={titnam;['(',num2str(pltensize),' mem, med.)']};   
