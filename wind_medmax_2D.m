...
%
 ...
 spd10_maxm=squeeze(max(spd10_ens_max,[],3));

%  if kicksea~=0;   spd10_max(land+1==1)=NaN; spd10_maxm(land+1==1)=NaN;  end
...
%
    ...
    % 
    tit={titnam;['(',num2str(pltensize),' mem, ens med)']};   
    ...
    %
    outfile=[outdir,'/',fignam,'kick',num2str(kicksea),'_m',num2str(pltensize),'rand',num2str(randmem),'med'];
    if saveid==1
    ...
    %
    outfile=[outdir,'/',fignam,'kick',num2str(kicksea),'_m',num2str(pltensize),'rand',num2str(randmem),'max'];
    if saveid==1
     print(hf,'-dpng',[outfile,'.png'])    
     system(['convert -trim ',outfile,'.png ',outfile,'.png']);
    end
